...
class ParticleEmitter extends Renderable {
  ...
  /// Pool of inactive particles for reuse (avoids GC pressure)
  ...
  void update(double deltaTime) {
    ...
    // Update existing particles — recycle dead ones into the pool
  ...
  void _emitParticle() {
    ...
    // Reuse a pooled particle when available, otherwise allocate a new one
    if (_pool.isNotEmpty) {
...
class Particle {
  ...
  /// Reusable paint object (avoids per-frame allocation)
  final Paint _paint = Paint()..style = PaintingStyle.fill;

  /// Reusable path for triangle / star shapes (avoids per-frame allocation)
  ...
  /// Reinitialize this particle for reuse from pool
  void reset({
  ...
  void update(double deltaTime, Offset gravity) {
    ...
    // Apply gravity + update velocity (single allocation)
    velocity = Offset(
      velocity.dx + gravity.dx * deltaTime,
      velocity.dy + gravity.dy * deltaTime,
    );

    // Update position (single allocation)
    position = Offset(
  ...
  void render(Canvas canvas) {
    ...
    switch (shape) {
      case ParticleShape.circle:
        canvas.drawCircle(position, size / 2, _paint);
        break;
      ...
      case ParticleShape.star:
        _drawStar(canvas, position, size / 2, _paint);
        break;
    }
  }
